...
To backup your database to Amazon S3, run the following from project root or set it up as a cron job to run automatically.  The following example keeps the last 3 backups in S3.

  rake db:backup RAILS_ENV=production

Optionally put this in a crontab to run the backup automatically.

  cd /path/to/your/project && /var/lib/gems/1.8/bin/rake db:backup RAILS_ENV=production

If you ever need to restore the database, use the tool of your choice to download the backup from S3 and run.
